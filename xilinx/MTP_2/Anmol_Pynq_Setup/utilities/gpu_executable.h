...
#include "gpu_types.h"
#include "non_atomically_upgradeable_rw_lock.h"
#include "executable.h"
#include "executable.h"
#include "thunk.h"
...
// Returns whether GpuExecutable runs with OpenXla Runtime.
...
class GpuExecutable : public Executable {
 public:
  ...
  using OwnedOpenXlaRuntimeProgram = std::unique_ptr<OpenXlaRuntimeProgram>;
  ...
  struct Params {
    ...
    // (native function) or OpenXLA runtime executable (IREE VM function)
    // depending on which is supplied.
    std::variant<OwnedThunkSequence, OwnedGpuRuntimeProgram,
                 OwnedOpenXlaRuntimeProgram>
        executable;
 ...
 private:
  ...
  // IrEmitter (null if Xla/OpenXla runtime is enabled).
  ...
  // OpenXLA executable that encapsulates all the state for running XLA:GPU
  // executables compiled to IREE VM modules (including VM module itself).
  std::unique_ptr<OpenXlaRuntimeExecutable> openxla_executable_;
